...
export default  function ReservationDetails({params}) {
    ...
    async function getReservationDetails(reservationId)
    {
        ...
        catch (error)
        {
            ...
            console.log("error lors de la recuperation des details de la reservation-details ",error);
        }
    }
        ...
        <div className="p-6">
            ...
            <div className="flex bg-gray-100 items-center mb-8 p-6 h-28 rounded-xl">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-reservation-color">Reservation Details</h1>
                        <p className="text-gray-500 font-medium mt-1">Booking Reference: {reservationDetails?.reservation?.idReservation}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-gray-100 rounded-lg  overflow-hidden">
                                ...
                                <div className="flex justify-between mb-7">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-bold text-md">{reservationDetails?.voyage?.titre} </p>
                                        </div>
                                    </div>

                                    <div className="flex gap-4 items-center mb-6">
                                        <span className={`flex justify-center items-center px-3 py-1 rounded-full text-sm font-medium ${reservationDetails?.reservation?.statutReservation === 'CONFIRMER' ? 'bg-green-100 text-green-600 border-2 border-green-300' : 'bg-orange-100 text-orange-600 border-2 border-orange-300'}`}>
                                            <FaExclamation/>
                                          {reservationDetails?.reservation?.statutReservation === 'CONFIRMER' ? "confirmed" : "waiting for confirmation"}
                                        </span>
                                        <span className="flex gap-0.5 justify-center items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 border-2 border-blue-300 text-reservation-color">
                                            ...
                                            {reservationDetails?.reservation?.classeVoyage || 'not specified'}
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.voyage?.nomAgence || 'not specified'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.voyage?.vehicule?.idVehicule || 'not specified'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.voyage?.dateDepartEffectif ? formatDateOnly(reservationDetails?.voyage?.dateDepartEffectif) : 'not specified'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.voyage?.heureDepartEffectif ? formatDateToTime(reservationDetails?.voyage?.heureDepartEffectif) : 'not specified'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Calendar className="text-reservation-color h-6 w-6"/>
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.voyage?.lieuDepart + " - " + reservationDetails?.voyage?.pointDeDepart || 'not specified'}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.voyage?.lieuArrive + " - " + reservationDetails?.voyage?.pointArrivee || 'not specified'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.passager?.length} passenger(s)</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{parseInt(reservationDetails?.reservation?.montantPaye)} FCFA</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold">{reservationDetails?.voyage?.dateLimiteConfirmation ? formatDateOnly(reservationDetails?.voyage?.dateLimiteConfirmation) : 'not specified'}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="bg-orange-50 p-4">
                                ...
                                <p className="text-md ml-10 text-justify text-orange-800">Cancellation policies vary by
                                    agency and take into account the time of year, ticket price and the time between
                                    cancellation and travel date. Depending on these criteria, cancellation fees may apply,
                                    or a coupon of varying value may be generated for future use.</p>
                            </div>
                        </div>
                                        ...
                                        <div key={index}
                                            ...
                                            <div className="space-y-3 text-sm mb-6">
                                                ...
                                                <p className="flex items-center">
                                                    ...
                                                    {parseInt(reservationDetails?.reservation?.prixTotal) / parseInt(reservationDetails?.reservation?.nbrPassager)} FCFA
                                                </span>
                                                </p>
                                            </div>
                                            <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                                                <button
                                                    ...
                                                    disabled={reservationDetails?.reservation?.statutReservation === 'RESERVER'}
                                                ...
                                                <button onClick={() => alert("cancellation")}
