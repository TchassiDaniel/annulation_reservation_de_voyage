...
export  function Sidebar()
{
    ...
    return (
        <div className="flex flex-col mt-4 mr-2">
           <Image src={logo} alt={"App logo"} className="w-48 h-24 ml-5"  />
            ...
            <nav className="mt-3 ml-6">
                <ul className="space-y-6">
                    {navLink.map((link, index) => {
                        ...
                        return (
                            <li key={index} className={isActive ? "bg-reservation-color p-3 rounded-lg text-base-color" : ""}>
                                <Link href={link.Link} className="flex gap-4">
                                    {IconComponent && <IconComponent  className={ isActive ? "text-white text-xl ": "text-gray-800 text-xl" }/>}
                                    <p>{link.name}</p>
