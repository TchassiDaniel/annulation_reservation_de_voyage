...
import com.annulation_reservation_voyage.annulation_reservation_voyage.services.VehiculeService;
...
public class VehiculeController {
    ...
    @Operation(summary = "Obtenir tous les vehicules",
            description = "Récupère la liste de tous les vehicules enregistrés.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",
                    description = "Liste récupérée avec succès",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = Vehicule.class))))
    })
    @GetMapping
    ...
    @Operation(summary = "Obtenir tous les vehicules d'une agence",
            description = "Récupère la liste de tous les vehicules de l'agence dont l'id est passé dans la route enregistrés.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",
                    description = "Liste récupérée avec succès",
                    content = @Content(mediaType = "application/json",
    ...
    public ResponseEntity<Page<Vehicule>> getAllVehiculesForAgence(@PathVariable UUID idAgence, @RequestParam(defaultValue = "0") int page,
                                                          @RequestParam(defaultValue = "10") int size) {
        Page<Vehicule> vehicules = vehiculeService.findByIdAgenceVoyage(idAgence, page, size);
        return new ResponseEntity<>(vehicules, HttpStatus.OK);
    }

    @Operation(summary = "Obtenir un vehicule par ID", description = "Récupère un vehicule en fonction de son identifiant.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Vehicule trouvé",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = Vehicule.class))),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Vehicule créé avec succès",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = Vehicule.class))),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Vehicule mis à jour avec succès",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = Vehicule.class))),
            @ApiResponse(responseCode = "404", description = "vehicule non trouvé"),
            @ApiResponse(responseCode = "400", description = "Données invalides")
    })
